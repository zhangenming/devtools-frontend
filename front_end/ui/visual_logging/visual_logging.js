...
function makeConfigStringBuilder(veName, context) {
  ...
  return {
    context: function(value2) {
      if (typeof value2 === "number" || typeof value2 === "string" && value2.length) {
        components.push(`context: ${value2}`);
      }
      checkContextValue(context);
      return this;
    },
    parent: function(value2) {
      components.push(`parent: ${value2}`);
      return this;
    },
    track: function(options) {
      components.push(`track: ${Object.entries(options).map(([key2, value2]) => value2 !== true ? `${key2}: ${value2}` : key2).join(", ")}`);
      return this;
    },
    toString: function() {
...
function deleteUndefinedFields(entry) {
  for (const stringKey in entry) {
    const key2 = stringKey;
    if (typeof entry[key2] === "undefined") {
      delete entry[key2];
    }
  }
}
...
function processImpressionsForTestDebugLog(states) {
  ...
  for (const state2 of states) {
    let key2 = "";
    if (state2.parent) {
      key2 = (veTestKeys.get(state2.parent.veid) || "<UNKNOWN>") + " > ";
    }
    key2 += VisualElements[state2.config.ve];
    if (state2.config.context) {
      key2 += ": " + state2.config.context;
    }
    veTestKeys.set(state2.veid, key2);
    lastImpressionLogEntry.impressions.push(key2);
  }
  checkPendingEventExpectation();
}
...
function debugString(config) {
  ...
  if (config.track) {
    components.push(`track: ${Object.entries(config.track).map(([key2, value2]) => `${key2}${typeof value2 === "string" ? `: ${value2}` : ""}`).join(", ")}`);
  }
  return components.join("; ");
}
...
function formatInteraction(e) {
  if ("interaction" in e) {
    if (e.veid !== void 0) {
      const key2 = veTestKeys.get(e.veid) || (e.veid ? "<UNKNOWN>" : "");
      return `${e.interaction}: ${key2}`;
    }
...
function contextFromKeyCodes(event) {
  ...
  const components = [];
  if (event.shiftKey && key2 !== lowerCaseKey) {
...
async function isUnderInspection(origin) {
  ...
  const context = await contextAsNumber(origin);
  if (!context) {
    return false;
  }
  return [431010711, -1313957874, -1093325535].includes(context);
}
function setHighlightedVe2(veKey) {
  setHighlightedVe(veKey);
  if (veKey) {
    void process();
  }
}
...
var canvas = makeConfigStringBuilder.bind(null, "Canvas");
var close = makeConfigStringBuilder.bind(null, "Close");
var colorEyeDropper = makeConfigStringBuilder.bind(null, "ColorEyeDropper");
var counter = makeConfigStringBuilder.bind(null, "Counter");
var controlPoint = makeConfigStringBuilder.bind(null, "ControlPoint");
...
var elementsBreadcrumbs = makeConfigStringBuilder.bind(null, "ElementsBreadcrumbs");
...
var mediaInspectorView = makeConfigStringBuilder.bind(null, "MediaInspectorView");
var menu = makeConfigStringBuilder.bind(null, "Menu");
var metricsBox = makeConfigStringBuilder.bind(null, "MetricsBox");
var paletteColorShades = makeConfigStringBuilder.bind(null, "PaletteColorShades");
var pane = makeConfigStringBuilder.bind(null, "Pane");
var panel = makeConfigStringBuilder.bind(null, "Panel");
var panelTabHeader = makeConfigStringBuilder.bind(null, "PanelTabHeader");
var pieChart = makeConfigStringBuilder.bind(null, "PieChart");
var pieChartSlice = makeConfigStringBuilder.bind(null, "PieChartSlice");
var pieChartTotal = makeConfigStringBuilder.bind(null, "PieChartTotal");
var popover = makeConfigStringBuilder.bind(null, "Popover");
var preview = makeConfigStringBuilder.bind(null, "Preview");
var resizer = makeConfigStringBuilder.bind(null, "Resizer");
var responsivePresets = makeConfigStringBuilder.bind(null, "ResponsivePresets");
var showStyleEditor = makeConfigStringBuilder.bind(null, "ShowStyleEditor");
var slider = makeConfigStringBuilder.bind(null, "Slider");
var section = makeConfigStringBuilder.bind(null, "Section");
var sectionHeader = makeConfigStringBuilder.bind(null, "SectionHeader");
var tableRow = makeConfigStringBuilder.bind(null, "TableRow");
var tableCell = makeConfigStringBuilder.bind(null, "TableCell");
var tableHeader = makeConfigStringBuilder.bind(null, "TableHeader");
...
export {
  ...
  key,
  link,
  logClick2 as logClick,
  logFunctionCall,
  logImpressions,
  logKeyDown2 as logKeyDown,
  logResize2 as logResize,
  logSettingAccess,
  mediaInspectorView,
  menu,
  metricsBox,
  ...
  popover,
  preview,
  registerLoggable2 as registerLoggable,
  registerParentProvider,
  resizer,
  responsivePresets,
  section,
  sectionHeader,
  setHighlightedVe2 as setHighlightedVe,
  setMappedParent,
  setVeDebugLoggingEnabled,
